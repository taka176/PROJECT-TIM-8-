...
export default function TodoInputDlu({ TambahData }: TodoInputProps) {
  ...
  const TangkapInput = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setMessage("Gagal menyimpan todo. Silakan coba lagi.");
      setSuccess(false);
    } finally {
    ...
    <div>
      <form onSubmit={TangkapInput} className="flex items-center gap-3 mb-2">
        <input
          ...
          className="flex-1 bg-stone-100/70 text-gray-800 placeholder-gray-400 text-sm px-4 py-3 rounded-2xl outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all disabled:opacity-50"
        />

        <button
          type="submit"
          disabled={isSubmitting || !text.trim()}
          className="w-11 h-11 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl flex items-center justify-center text-xl font-medium transition-colors shadow-sm shrink-0 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isSubmitting ? "..." : "+"}
        </button>
      </form>
